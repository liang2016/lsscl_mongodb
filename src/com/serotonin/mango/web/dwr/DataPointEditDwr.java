...
public class DataPointEditDwr extends BaseDwr {
	...
	//根据id查询demo
	public DwrResponseI18n setDome(int id) {
		DwrResponseI18n response = new DwrResponseI18n();
		User user = Common.getUser();
		RenderVo render=new TextRenderDao().findById(id);
		DataPointVO dp =user.getEditPoint(); 
		int type= dp.getPointLocator().getDataTypeId();
		if(render.getDataType()==type){
			dp.setTextRenderer(render.getTextRenderer());
			response.addData("dataType",render.getDataType());
			response.addData("textRenderer",dp.getTextRenderer());
		}
		else{
			response.addMessage(new LocalizableMessage(
			"pointEdit.text.dataType.error"));
		}
		return response;
	}
