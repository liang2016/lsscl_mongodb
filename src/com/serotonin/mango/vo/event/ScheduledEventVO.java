 ...
 */
@JsonRemoteEntity
public class ScheduledEventVO extends SimpleEventDetectorVO implements
		ChangeComparable<ScheduledEventVO>, JsonSerializable {
	...
	static {
		TYPE_CODES.addElement(TYPE_HOURLY, "HOURLY",
				"scheduledEvents.type.hour");
		TYPE_CODES.addElement(TYPE_DAILY, "DAILY", "scheduledEvents.type.day");
		TYPE_CODES.addElement(TYPE_WEEKLY, "WEEKLY",
				"scheduledEvents.type.week");
		TYPE_CODES.addElement(TYPE_MONTHLY, "MONTHLY",
				"scheduledEvents.type.month");
		TYPE_CODES.addElement(TYPE_YEARLY, "YEARLY",
				"scheduledEvents.type.year");
	...
	public EventTypeVO getEventType() {
		return new EventTypeVO(EventType.EventSources.SCHEDULED, id, 0,
				getDescription(), alarmLevel, getEventDetectorKey());
	}
	...
	public LocalizableMessage getDescription() {
		...
		if (!StringUtils.isEmpty(alias))
			...
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.onceUntil",
						DateFunctions.getTime(new DateTime(activeYear,
								activeMonth, activeDay, activeHour,
								activeMinute, activeSecond, 0).getMillis()),
						DateFunctions.getTime(new DateTime(inactiveYear,
								inactiveMonth, inactiveDay, inactiveHour,
								inactiveMinute, inactiveSecond, 0).getMillis()));
			else
				message = new LocalizableMessage("event.schedule.onceAt",
						DateFunctions.getTime(new DateTime(activeYear,
								activeMonth, activeDay, activeHour,
								activeMinute, activeSecond, 0).getMillis()));
		} else if (scheduleType == TYPE_HOURLY) {
			String activeTime = StringUtils.pad(Integer.toString(activeMinute),
					'0', 2)
					+ ":"
					+ StringUtils.pad(Integer.toString(activeSecond), '0', 2);
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.hoursUntil",
						activeTime, StringUtils.pad(Integer
								.toString(inactiveMinute), '0', 2)
								+ ":"
								+ StringUtils.pad(Integer
										.toString(inactiveSecond), '0', 2));
			else
				message = new LocalizableMessage("event.schedule.hoursAt",
						activeTime);
		} else if (scheduleType == TYPE_DAILY) {
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.dailyUntil",
						activeTime(), inactiveTime());
			else
				message = new LocalizableMessage("event.schedule.dailyAt",
						activeTime());
		} else if (scheduleType == TYPE_WEEKLY) {
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.weeklyUntil",
						weekday(true), activeTime(), weekday(false),
						inactiveTime());
			else
				message = new LocalizableMessage("event.schedule.weeklyAt",
						weekday(true), activeTime());
		} else if (scheduleType == TYPE_MONTHLY) {
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.monthlyUntil",
						monthday(true), activeTime(), monthday(false),
						inactiveTime());
			else
				message = new LocalizableMessage("event.schedule.monthlyAt",
						monthday(true), activeTime());
		} else if (scheduleType == TYPE_YEARLY) {
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.yearlyUntil",
						monthday(true), month(true), activeTime(),
						monthday(false), month(false), inactiveTime());
			else
				message = new LocalizableMessage("event.schedule.yearlyAt",
						monthday(true), month(true), activeTime());
		} else if (scheduleType == TYPE_CRON) {
			if (returnToNormal)
				message = new LocalizableMessage("event.schedule.cronUntil",
						activeCron, inactiveCron);
			else
				message = new LocalizableMessage("event.schedule.cronAt",
						activeCron);
		} else
			throw new ShouldNeverHappenException("Unknown schedule type: "
					+ scheduleType);
	...
	private static final String[] weekdays = { "", "common.day.mon",
			"common.day.tue", "common.day.wed", "common.day.thu",
			"common.day.fri", "common.day.sat", "common.day.sun" };
	...
	private LocalizableMessage monthday(boolean active) {
		...
		if (day != 11 && day % 10 == 1)
			return new LocalizableMessage("common.counting.st", Integer
					.toString(day));
		if (day != 12 && day % 10 == 2)
			return new LocalizableMessage("common.counting.nd", Integer
					.toString(day));
		if (day != 13 && day % 10 == 3)
			return new LocalizableMessage("common.counting.rd", Integer
					.toString(day));
		return new LocalizableMessage("common.counting.th", Integer
				.toString(day));
	}

	private static final String[] months = { "", "common.month.jan",
			"common.month.feb", "common.month.mar", "common.month.apr",
			"common.month.may", "common.month.jun", "common.month.jul",
			"common.month.aug", "common.month.sep", "common.month.oct",
			"common.month.nov", "common.month.dec" };
	...
	public void validate(DwrResponseI18n response) {
		if (StringUtils.isLengthGreaterThan(alias, 50))
			response.addContextualMessage("alias",
					"scheduledEvents.validate.aliasTooLong");
		...
		if (scheduleType == TYPE_CRON) {
			try {
				...
				response.addContextualMessage("activeCron",
						"scheduledEvents.validate.activeCron", e.getMessage());
			}
			...
			if (returnToNormal) {
				try {
					...
					response.addContextualMessage("inactiveCron",
							"scheduledEvents.validate.inactiveCron", e
									.getMessage());
				}
			}
		}
		...
		try {
			...
			response.addContextualMessage("activeCron",
					"scheduledEvents.validate.activeTrigger", e.getMessage());
		}
		...
		if (returnToNormal) {
			try {
				...
				response.addContextualMessage("inactiveCron",
						"scheduledEvents.validate.inactiveTrigger", e
								.getMessage());
			}
		}

		// If the event is once, make sure the active time is earlier than the
		// inactive time.
		if (scheduleType == TYPE_ONCE && returnToNormal) {
			DateTime adt = new DateTime(activeYear, activeMonth, activeDay,
					activeHour, activeMinute, activeSecond, 0);
			DateTime idt = new DateTime(inactiveYear, inactiveMonth,
					inactiveDay, inactiveHour, inactiveMinute, inactiveSecond,
					0);
			if (idt.getMillis() <= adt.getMillis())
				response.addContextualMessage("scheduleType",
						"scheduledEvents.validate.invalidRtn");
		}
		
		if(scheduleType==ScheduledEventVO.TYPE_ONCE){
        	DateTime start=new DateTime(activeYear,activeMonth,activeDay,activeHour,activeMinute,activeSecond,0);
        	if(start.getMillis()<System.currentTimeMillis())
        		response.addContextualMessage("scheduleType", "scheduledEvents.validate.invalidRtn2");
        }
	}
	...
	public void addProperties(List<LocalizableMessage> list) {
		...
		AuditEventType.addPropertyMessage(list, "common.alarmLevel",
				AlarmLevels.getAlarmLevelMessage(alarmLevel));
		AuditEventType.addPropertyMessage(list, "scheduledEvents.type",
				getTypeMessage());
		...
		AuditEventType.addPropertyMessage(list, "common.configuration",
				getDescription());
	}
	...
	public void addPropertyChanges(List<LocalizableMessage> list,
		...
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"scheduledEvents.alias", from.alias, alias);
		AuditEventType.maybeAddAlarmLevelChangeMessage(list,
				"common.alarmLevel", from.alarmLevel, alarmLevel);
		if (from.scheduleType != scheduleType)
			AuditEventType.addPropertyChangeMessage(list,
					"scheduledEvents.type", from.getTypeMessage(),
					getTypeMessage());
		AuditEventType.maybeAddPropertyChangeMessage(list, "common.rtn",
				from.returnToNormal, returnToNormal);
		AuditEventType.maybeAddPropertyChangeMessage(list, "common.disabled",
				from.disabled, disabled);
		if (from.activeYear != activeYear || from.activeMonth != activeMonth
				|| from.activeDay != activeDay || from.activeHour != activeHour
				|| from.activeMinute != activeMinute
				|| from.activeSecond != activeSecond
				|| from.activeCron != activeCron
				|| from.inactiveYear != inactiveYear
				|| from.inactiveMonth != inactiveMonth
				|| from.inactiveDay != inactiveDay
				|| from.inactiveHour != inactiveHour
				|| from.inactiveMinute != inactiveMinute
				|| from.inactiveSecond != inactiveSecond
				|| from.inactiveCron != inactiveCron)
			AuditEventType.maybeAddPropertyChangeMessage(list,
					"common.configuration", from.getDescription(),
					getDescription());
	}
	...
	public void jsonDeserialize(JsonReader reader, JsonObject json)
		...
		if (text != null) {
			...
			if (!AlarmLevels.CODES.isValidId(alarmLevel))
				throw new LocalizableJsonException(
						"emport.error.scheduledEvent.invalid", "alarmLevel",
						text, AlarmLevels.CODES.getCodeList());
		}
		...
		if (text != null) {
			...
			if (!TYPE_CODES.isValidId(scheduleType))
				throw new LocalizableJsonException(
						"emport.error.scheduledEvent.invalid", "scheduleType",
