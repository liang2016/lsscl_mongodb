...
public class MailingListDao extends BaseDao {
    ...
    public List<MailingList> getMailingLists(int scopeId) {
        List<MailingList> result = query(MAILING_LIST_SELECT + "where scopeid=? order by name",new Object[]{scopeId}, new MailingListRowMapper());
        setRelationalData(result);
        return result;
    }
    ...
    private static final String MAILING_LIST_INSERT = "insert into mailingLists (xid, name,scopeId) values (?,?,?)";
        ...
        getTransactionTemplate().execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                if (ml.getId() == Common.NEW_ID)
                    ml.setId(doInsert(MAILING_LIST_INSERT, new Object[] { ml.getXid(), ml.getName(),ml.getScopeId() }));
                else
