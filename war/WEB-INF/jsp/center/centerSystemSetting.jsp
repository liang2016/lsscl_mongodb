...
<tag:page dwr="SystemSettingsDwr" js="emailRecipients" onload="init">
 <script type="text/javascript" src="dwr/interface/SendSettingDwr.js"></script>
  <script type="text/javascript">
    ...
    function init() {
        ...
        SystemSettingsDwr.getSettings(function(settings) {
       		$set("<c:out value="<%= SystemSettingsDao.EMAIL_TYPE %>"/>", settings.<c:out value="<%= SystemSettingsDao.EMAIL_TYPE %>"/>);
        	if(settings.<c:out value="<%= SystemSettingsDao.EMAIL_TYPE %>"/>=='exchange'){
        		hide("smtp");
        	}
			else{
				hide("exchange");
            ...
            $set("<c:out value="<%= SystemSettingsDao.EXCHANGE_URL %>"/>", settings.<c:out value="<%= SystemSettingsDao.EXCHANGE_URL %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EXCHANGE_USERNAME %>"/>", settings.<c:out value="<%= SystemSettingsDao.EXCHANGE_USERNAME %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EXCHANGE_PASSWORD %>"/>", settings.<c:out value="<%= SystemSettingsDao.EXCHANGE_PASSWORD %>"/>);
            $set("<c:out value="<%= SystemSettingsDao.EXCHANGE_DOMAIN %>"/>", settings.<c:out value="<%= SystemSettingsDao.EXCHANGE_DOMAIN %>"/>);
	         ...
	         var users=settings.adminUsers;
			   for(var i=0;i<users.length;i++){
				   ...
				   var checkTd2=document.createElement("td");
			   	  checkTd.innerHTML="<img src='images/user_suit.png' title='<fmt:message key="common.administrator"/>' alt='<fmt:message key="common.administrator"/>'>"+users[i].username;
			   	  checkTd1.innerHTML="<input size='5px' name='limitValue' class='formField' type='text' id='u"+users[i].homeScope.id+"' value='"+users[i].limit+"'/>";
			   	  checkTd2.innerHTML="<input class='formField' type='button' onclick='saveUserLimit("+users[i].homeScope.id+")' value='<fmt:message key="common.save"/>'/>";
			   	  checkTr.appendChild(checkTd);
				  checkTr.appendChild(checkTd1);
				  checkTr.appendChild(checkTd2);
				  $("factoryUsers").appendChild(checkTr);
			   	}
			   
			 var adminUsers=settings.adminAddUsers;
			   for(var i=0;i<adminUsers.length;i++){
			       var checkTr=document.createElement("tr");
				   var checkTd=document.createElement("td");
				   var checkTd1=document.createElement("td");
				   var checkTd2=document.createElement("td");
			   	  checkTd.innerHTML="<img src='images/user_suit.png' title='<fmt:message key="common.administrator"/>' alt='<fmt:message key="common.administrator"/>'>"+adminUsers[i].username;
			   	  checkTd1.innerHTML="<input size='5px' name='addLimitValue' class='formField' type='text' id='l"+adminUsers[i].homeScope.id+"' value='"+adminUsers[i].limit+"'/>";
			   	  checkTd2.innerHTML="<input class='formField' type='button' onclick='saveUserAddLimit("+adminUsers[i].homeScope.id+")' value='<fmt:message key="common.save"/>'/>";
			   	  checkTr.appendChild(checkTd);
				  checkTr.appendChild(checkTd1);
				  checkTr.appendChild(checkTd2);
				  $("factoryUsers2").appendChild(checkTr);
			   	}
        });
    }
     ...
     function initCB(data){
    	...
    	escalRecipients.write("escalRecipients", "escalRecipients", "escalationAddresses",
        		"<sst:i18n key="eventHandlers.escal2" escapeDQuotes="true"/>");
   		
   $("id").value=data.setting.id;
  		 escalRecipients.updateRecipientList(data.setting.escalationRecipients2);
  }
  
    function saveUserLimit(userId){
	  	var limitInput="u"+userId;
	  	var limit=parseInt($get(limitInput));
	  	SendSettingDwr.updateUserEventhandlerLimit(userId,limit,function(response){
	  		if (!response.hasMessages)
	  		setUserLimitError("<fmt:message key="users.handlerLimit.saved"/>");
	  		else{
	  		  hide("userLimitMessage");
	  		  showDwrMessages(response.messages, $("userLimitErrorMessage"));
	  		  }
	  	 });
  }
      function saveUserAddLimit(userId){
	  	var limitInput="l"+userId;
	  	var limit=parseInt($get(limitInput));
	  	SystemSettingsDwr.updateUserAddLimit(userId,limit,function(response){
	  		if (!response.hasMessages)
	  		setUserAddLimitError("<fmt:message key="users.handlerLimit.saved"/>");
	  		else{
	  		  hide("userLimitMessage");
	  		  showDwrMessages(response.messages, $("userErrorMessage"));
	  		  }
	  	 });
  }
  function saveAllUserLimit(){
  	 var userTable=document.getElementsByName("limitValue");
  	 var usersLimits=new Array(new Array());
  	 for (var i=0; i<userTable.length; i++) {
		usersLimits[i]=new Array((userTable[i].id).substr(1),userTable[i].value);
	}
	SendSettingDwr.updateAllUserEventhandlerLimit(usersLimits,function (response){
	 if (!response.hasMessages)
	    setUserLimitError("<fmt:message key="users.handlerLimit.all.saved"/>");
	  else{
	    hide("userLimitMessage");
	  	showDwrMessages(response.messages, $("userLimitErrorMessage"));
	  	}
  ...
  function saveAddUserLimit(){
	  var userTable=document.getElementsByName("addLimitValue");
	  	 var usersLimits=new Array(new Array());
	  	 for (var i=0; i<userTable.length; i++) {
			usersLimits[i]=new Array((userTable[i].id).substr(1),userTable[i].value);
		}
		SystemSettingsDwr.updateAllUserAddLimit(usersLimits,function (response){
		 if (!response.hasMessages)
		    setUserAddLimitError("<fmt:message key="users.handlerLimit.all.saved"/>");
		  else{
		    hide("userMessage");
		  	showDwrMessages(response.messages, $("userErrorMessage"));
		  	}
  ...
  function saveSendSetting(){
   ...
   var escalList = escalRecipients.createRecipientArray();
   	SendSettingDwr.saveSendSetting($get("id"),$get("scopeId"),0,escalList,saveEventHandlerCB);
  }
    ...
    function setEventTypeData(listId, eventTypes, alarmFunctions, alarmOptions, alarmLevelsList) {
     ...
     function saveEventHandlerCB(response) {
        if (response.hasMessages){
            ...
            setUserMessage("userMessage1","");
            }
        else {
        	hide("genericMessages");
             var handler = response.data.handler;
             setUserMessage("userMessage1","<fmt:message key="eventHandlers.saved"/>");
              $("id").value=response.data.handler.id;
           escalRecipients.updateRecipientList(response.data.handler.escalationRecipients2);
    ...
    function saveEmailSettings() {
        SystemSettingsDwr.saveEmailSettings(
            ...
            $get("<c:out value="<%= SystemSettingsDao.EMAIL_TYPE %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EXCHANGE_URL %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EXCHANGE_USERNAME %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EXCHANGE_PASSWORD %>"/>"),
            $get("<c:out value="<%= SystemSettingsDao.EXCHANGE_DOMAIN %>"/>"),
            function() {
    ...
    */
    function checkPurgeAllData() {
  	...
  	function setUserLimitError(msg){
    	hideGenericMessages("userLimitErrorMessage");
    	showMessage("userLimitMessage", msg);
    }
   function setUserAddLimitError(msg){
    	hideGenericMessages("userErrorMessage");
    	showMessage("userMessage", msg);
    }
    function showEmailType(id){
    	if(id=='exchange'){
    		hide("smtp");
    		show("exchange");
    	}
  ...
  <div class="borderDiv marB marR" style="float:left">
    ...
    <table>
      <tr>
        <td>
        <span class="formLabelRequired"><fmt:message key="systemSettings.emailtype"/></span>
        </td>
        <td class="formField">
          <select id="emailType" onchange="showEmailType(this.value)">
            <option value="exchange"/><fmt:message key="systemSettings.email.exchange"/></option>
            <option value="smtp"/><fmt:message key="systemSettings.email.smtp"/></option>
          </select>
        </td>
       </tr>
   </table>
    ...
    <table id="exchange">
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.exchangeUrl"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EXCHANGE_URL %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.exchangeUsername"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EXCHANGE_USERNAME %>"/>" type="text"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.exchangePassword"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EXCHANGE_PASSWORD %>"/>" type="password"/></td>
      </tr>
      <tr>
        <td class="formLabelRequired"><fmt:message key="systemSettings.exchangeDomain"/></td>
        <td class="formField"><input id="<c:out value="<%= SystemSettingsDao.EXCHANGE_DOMAIN %>"/>" type="text"/></td>
      </tr>
    </table>
 ...
 -->  
  ...
  <div class="borderDiv marB marR" style="float:left">
    ...
    <div class="borderDiv marB marR" style="float:left">
  <table id="handler<c:out value="<%=EventHandlerVO.TYPE_EMAIL%>"/>"width="100%">
		<tbody>
			<tr align="right">
				<td>
			...
			<tr>
				<td class="formError" id="userMessage1"></td>
			</tr>
		</tbody>
  ...
  <div class="borderDiv marB marR" style="float:left">
	<table width="100%">
		<thead>
			<tr>
				<td>
					<span class="smallTitle"><fmt:message key="users.handlerLimit"/><tag:help id="handlerLimit"/></span>
				</td>
				<td align="right">
					<tag:img id="saveImg" png="save" title="common.save"
						onclick="saveAllUserLimit()" />
		   		</td>
	    	</tr>
	    	<tr>
	    	 <td colspan="2">
	    	  <span class="formError" id="userLimitMessage"></span>
	    	 </td>
	    	</tr>
	    	<tr>
	    	 <td colspan="2">
	   			<tbody id="userLimitErrorMessage"></tbody>
	   			</td>
	   		</tr>
		</thead>
		<tbody id="factoryUsers">
		</tbody>
	</table>
</div>
...
<div class="borderDiv marB marR" style="float:left">
	<table width="100%">
		<thead>
			<tr>
				<td>
					<span class="smallTitle"><fmt:message key="users.addLimit"/><tag:help id="userLimit"/></span>
				</td>
				<td align="right">
					<tag:img id="saveImg" png="save" title="common.save"
						onclick="saveAddUserLimit()" />
		   		</td>
	    	</tr>
	    	<tr>
	    	 <td colspan="2">
	    	  <span class="formError" id="userMessage"></span>
	    	 </td>
	    	</tr>
	    	<tr>
	    	 <td colspan="2">
	   			<tbody id="userErrorMessage"></tbody>
	   			</td>
	   		</tr>
		</thead>
		<tbody id="factoryUsers2">
		</tbody>
	</table>
</div>
