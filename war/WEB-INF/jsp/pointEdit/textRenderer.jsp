...
<div class="borderDiv marB marR">
  <table>
    ...
    <tr>
    	<td colspan="3"><table><tbody id="genericMessages"></tbody></table></td>
    </tr>
    <tr>
    	 <td class="formLabelRequired"><fmt:message key="pointEdit.text.demo"/></td>
		<td class="formField" colspan="2">
			<select id="textRenderDemo"></select> 
			<tag:img id="imageChartImg" png="control_play_blue" title="pointEdit.text.demo.run"
                 onclick="useDemo()"/>
		</td>
	</tr>
...
<script type="text/javascript">
  ...
  dojo.addOnLoad(textRendererEditor, "init");
	...
	function addDemo(){
		DataPointEditDwr.findAll(allDB);
	}
   function allDB(renders){
   		var renderSelect=$("textRenderDemo");
   		renderSelect.length=0;
		for(var i=0;i<renders.length;i++){
		    renderSelect[renderSelect.length] = new Option(renders[i].name,renders[i].id);
		 }
	}
	function useDemo(){
		if(window.rangeValues)
			rangeValues.length=0;
	  	hideGenericMessages("genericMessages");
		var demoId=$get("textRenderDemo");
		DataPointEditDwr.setDome(demoId,editDetailsCB);
	}
	function editDetailsCB(response){
	  if (response.hasMessages){
			...
			DataPointEditDwr.showTextRenderer(response.data.dataType,showTextRendererCB);
			}
	}
