...
<tag:page dwr="DataSourceListDwr" onload="init">
  <script>
    ...
    function deleteDataSource(dataSourceId) {
    	temp = dataSourceId;
        DataSourceListDwr.hasAcp(dataSourceId,deleteDataSourceValidate);
    }
    
    //validate before delete dataSource
    function deleteDataSourceValidate(data){
    	var dataSourceId = temp;
    	if(data){
    		if (confirm("<fmt:message key="dsList.dsDeleteValidate"/>")) {
    			return;
    		}
    	}else{
    ...
    function addDataSource() {
        window.location = "data_source_edit.shtm?typeId="+ $get("dataSourceTypes")+"&factoryId="+<c:out value="${sessionUser.currentScope.id}" />;
    }
  </script>
              ...
              <table cellspacing="1" cellpadding="0" border="0">
                ...
                <c:forEach items="${paging.data}" var="listParent" begin="${paging.offset}" end="${paging.endIndex}">
                  <tr class="row" id="dataSourceRow${listParent.parent.id}">
                    ...
                    <td>
                      <a href="data_source_edit.shtm?dsid=${listParent.parent.id}&factoryId=${sessionUser.currentScope.id}"><tag:img png="icon_ds_edit"
                              title="common.edit"/></a>
                        ...
                        <c:forEach items="${listParent.list}" var="point">
                          <tr id="pointRow${point.id}">
                            ...
                            <td><tag:img png="icon_comp" title="events.pointDetails"
                                    onclick="window.location='data_point_details.shtm?dpid=${point.id}&factoryId=${sessionUser.currentScope.id}'"/></td>
                            <td><tag:img png="icon_comp_edit" title="pointEdit.props.props"
                                    onclick="window.location='data_point_edit.shtm?dpid=${point.id}&factoryId=${sessionUser.currentScope.id}'"/></td>
                          </tr>
                        </c:forEach>
                      </table>
                    </td>
