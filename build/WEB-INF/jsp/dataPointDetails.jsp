...
<tag:page dwr="DataPointDetailsDwr" js="view" onload="init">
  <script type="text/javascript">
    ...
    function init() {
    	//show or hide the pid of url paramters
    	DataPointDetailsDwr.isSettable(<c:out value="${point.id}"></c:out>,function(settable){
    		if(settable){ 
    		<c:if test="${sessionUser.homeScope.scopetype!=3}">
    		if(null!=$("ds_dp_link"))
    			$("ds_dp_link").href+="&pid=${point.id}";
    		</c:if>
    		}
    	});
  ...
  <table width="100%">
    <tr>
      <td valign="top" align="right">
        ...
        <sst:select value="${point.id}" onchange="window.location='data_point_details.shtm?dpid='+ this.value+'&factoryId=${sessionUser.currentScope.id}'">
        ...
        <c:if test="${!empty prevId}">
          <tag:img png="bullet_go_left" title="pagination.previous"
                  onclick="window.location='data_point_details.shtm?dpid=${prevId}&factoryId=${sessionUser.currentScope.id}'"/>
        </c:if>
        
        <c:if test="${!empty nextId}">
          <tag:img png="bullet_go" title="pagination.next"
                  onclick="window.location='data_point_details.shtm?dpid=${nextId}&factoryId=${sessionUser.currentScope.id}'"/>
        </c:if>
      </td>
    </tr>
  </table>
  ...
  <table width="100%" cellspacing="0" cellpadding="0">
    <tr>
            ...
            <tr>
              <td class="smallTitle" colspan="2">
                ...
                <c:if test="${pointEditor}">
                  <a href="data_point_edit.shtm?dpid=${point.id}&factoryId=${sessionUser.currentScope.id}"><tag:img png="icon_comp_edit" title="pointDetails.editPoint"/></a>
                  <a id="ds_dp_link" href="data_source_edit.shtm?dsid=${point.dataSourceId}&factoryId=${sessionUser.currentScope.id}"><tag:img png="icon_ds_edit"
      ...
      <td valign="top">
        <div class="borderDiv marB marR" style="height:270px; overflow:auto">
          <table width="100%">
            <tr>
        ...
        <div class="borderDiv marR">
          ...
          <table width="100%" cellspacing="1">
            ...
            <c:forEach items="${views}" var="view" varStatus="status">
              <tr class="row<c:if test="${status.index % 2 == 1}">Alt</c:if>">
                <td>${view.name}</td>
                <td align="center"><a href="views.shtm?viewId=${view.id}&factoryId=${sessionUser.currentScope.id}"><tag:img png="icon_view"
