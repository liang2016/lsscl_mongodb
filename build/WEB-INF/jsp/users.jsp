    ...
    function saveUser() {
        ...
        if (adminUser) {
            ...
            UsersDwr.saveUserAdmin(editingUserId, $get("username"), $get("password"), $get("email"), $get("phone"), 
                    ...
                    dsPermis, dpPermis,$get("scopeTypeid"),saveUserCB);
        }
        else{
    ...
    <tr>
      <td valign="top" id="userList" style="display:none;">
        <div class="borderDiv">
          <table width="100%">
            <tr>
              <td><input type="hidden" id="scopeTypeid" value="${sessionUser.currentScope.scopetype}">
                <span class="smallTitle"><fmt:message key="users.title"/></span>
                <tag:help id="userAdministration"/>
              </td>
              <td align="right">
              	<tag:img png="user_add" onclick="showUser(${applicationScope['constants.Common.NEW_ID']})"
                      title="users.add" id="u${applicationScope['constants.Common.NEW_ID']}Img"/>
                      </td>
            </tr>
          </table>
          <table id="usersTable">
