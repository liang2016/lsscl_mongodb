 ...
 */
@JsonRemoteEntity
public class CompoundEventDetectorVO implements
		ChangeComparable<CompoundEventDetectorVO>, JsonSerializable {
	...
	public EventTypeVO getEventType() {
		return new EventTypeVO(EventType.EventSources.COMPOUND, id, 0,
				new LocalizableMessage("common.default", name), alarmLevel);
	}
	...
	public void validate(DwrResponseI18n response) {
		if (StringUtils.isEmpty(name))
			response.addContextualMessage("name",
					"compoundDetectors.validation.nameRequired");
	...
	public static void validate(String condition, DwrResponseI18n response) {
		try {
			...
			LogicalOperator l = CompoundEventDetectorRT
					.parseConditionStatement(condition);
			...
			for (String key : keys) {
				if (!key
						.startsWith(SimpleEventDetectorVO.POINT_EVENT_DETECTOR_PREFIX))
					continue;

				boolean found = false;
				for (DataPointVO dp : dataPoints) {
					if (!Permissions.hasDataSourcePermission(user, dp
							.getDataSourceId()))
						continue;

					for (PointEventDetectorVO ped : dp.getEventDetectors()) {
						if (ped.getEventDetectorKey().equals(key)
								&& ped.isRtnApplicable()) {
							found = true;
							break;
						}
				...
				if (!found)
					throw new ConditionParseException(new LocalizableMessage(
							"compoundDetectors.validation.invalidKey"));
			}
	...
	public void addProperties(List<LocalizableMessage> list) {
		...
		AuditEventType.addPropertyMessage(list, "compoundDetectors.condition",
				condition);
	}
	...
	public void addPropertyChanges(List<LocalizableMessage> list,
		...
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"compoundDetectors.name", from.name, name);
		...
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"compoundDetectors.condition", from.condition, condition);
	}
