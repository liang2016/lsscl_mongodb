...
<tag:page dwr="ReportsDwr" js="emailRecipients" onload="init">
  <script type="text/javascript">
    ...
    function loadReportCB(report) {
        ...
        $set("intervalTime", report.intervalTime);
        $set("timeType", report.timeType);
        $set("timeNum", report.timeNum);
    ...
    function viewChart(instanceId) {
        window.open("reportChart.shtm?instanceId="+ instanceId+"&factoryId=${sessionUser.currentScope.id}", "chartTarget");
    }
    ...
    function updateSelectPeriodFields(){
        var schedulePeriod = $("intervalTime");
        if(schedulePeriod.checked){
          setDisabled("timeType",false);
          setDisabled("timeNum",false);
        }
        else{
          setDisabled("timeType",true);
          setDisabled("timeNum",true);
        }
        //setDisabled("selectTime",schedulePeriod == <c:out value="<%= ReportVO.SCHEDULE_CRON %>"/>);
    ...
    function saveReport() {
    if($get("dateRangeType")==2){
    	var from=new Date($get("fromYear"),$get("fromMonth"), $get("fromDay"), $get("fromHour"), $get("fromMinute"));
    	var to=new Date($get("toYear"), $get("toMonth"), $get("toDay"), $get("toHour"), $get("toMinute"));
    	if(to.getTime()-from.getTime()>86400000){
    		showMessage("userMessage", "<fmt:message key="reports.time.overFolw"/>");
    		return;
    	}
    	
    }
     if(getReportPointIdsArray().length>5){
        	alert("<fmt:message key="watchlist.point.more.than.5"/>");
        	return;
        }   
        ReportsDwr.saveReport(selectedReport.id,$get("factoryId"),  $get("name"), getReportPointIdsArray(), $get("includeEvents"),
                ...
                $get("includeData"), $get("zipData"),$get("intervalTime"),$get("timeType"),$get("timeNum"), emailRecipients.createRecipientArray(), function(response) {
    ...
    function runReport() {
        ...
        ReportsDwr.runReport($get("name"),$get("factoryId"), getReportPointIdsArray(), $get("includeEvents"),
                $get("includeUserComments"), $get("dateRangeType"), $get("relativeType"), $get("prevPeriodCount"),
                $get("prevPeriodType"), $get("pastPeriodCount"), $get("pastPeriodType"), $get("fromNone"),
                $get("fromYear"), $get("fromMonth"), $get("fromDay"), $get("fromHour"), $get("fromMinute"),
                $get("toNone"), $get("toYear"), $get("toMonth"), $get("toDay"), $get("toHour"), $get("toMinute"),
                $get("email"), $get("includeData"), $get("zipData"),$get("intervalTime"),$get("timeType"),$get("timeNum"), emailRecipients.createRecipientArray(), function(response) {
    ...
    <tr>
      ...
      <td valign="top" id="reportDetails" style="display:none;">
        <div class="borderDiv">
          ...
          <table>
            <tr>
              <td class="formLabelRequired"><fmt:message key="reports.reportName"/>
               <input id="factoryId" type="hidden" value="${sessionUser.currentScope.id}"> 
              </td>
              <td class="formField">
            ...
            <tr><td colspan="3" class="horzSeparator"></td></tr>
             <tr>
            	<td class="formLabelRequired"><fmt:message key="reports.time.select"/></td>
            	<td class="formField">
            		<table cellpadding="0" cellspacing="0">
            		<tr>
		            <td class="formField">
		                <input type="checkbox" id="intervalTime" onclick="updateSelectPeriodFields()"/>
		            </td>
            		</tr>
                    <tr><td>
                      <select id="timeType" onchange="">
                        <option value="9"><fmt:message key="reports.timeDelay.minute"/></option>
                        <tag:timePeriodOptions h="true" d="true" w="true" mon="true" y="true" singular="true"/>
                      </select>
                    </td></tr>
                    <tr><td style="padding-left:40px;">
                      <fmt:message key="reports.timeDelay"/>: <input type="text" id="timeNum" disabled="disabled" class="formVeryShort"/>
                      <div id="timeDelayMinutesError" class="formError"></div>
                    </td></tr>
                    </table>
            	</td>
              </tr>
