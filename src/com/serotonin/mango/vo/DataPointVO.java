...
public class DataPointVO implements Serializable, Cloneable, JsonSerializable,
	...
	static {
		for (int i = 0; i < 190; i++)
			ENGINEERING_UNITS_CODES.addElement(i, StringUtils
					.capitalize(new EngineeringUnits(i).toString()), "engUnit."
					+ i);
	}
	...
	private int loggingType = LoggingTypes.ALL;
	...
	public void addProperties(List<LocalizableMessage> list) {
		...
		AuditEventType.addPropertyMessage(list, "pointEdit.logging.discardLow",
				discardLowLimit);
		AuditEventType.addPropertyMessage(list,
				"pointEdit.logging.engineeringUnits", engineeringUnits);
	...
	public void addPropertyChanges(List<LocalizableMessage> list,
		...
		AuditEventType.maybeAddPropertyChangeMessage(list,
				...
				discardHighLimit);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"pointEdit.logging.engineeringUnits", from.engineeringUnits,
				engineeringUnits);
		AuditEventType.maybeAddPropertyChangeMessage(list,
	...
	public String toString() {
		return "DataPointVO [id=" + id + ", xid=" + xid + ", name=" + name
				...
				+ ", tolerance=" + tolerance + ", purgeType=" + purgeType
				+ ", purgePeriod=" + purgePeriod + ", textRenderer="
				+ textRenderer + ", chartRenderer=" + chartRenderer
				+ ", eventDetectors=" + eventDetectors + ", comments="
				+ comments + ", defaultCacheSize=" + defaultCacheSize
				+ ", discardExtremeValues=" + discardExtremeValues
				+ ", discardLowLimit=" + discardLowLimit
				+ ", discardHighLimit=" + discardHighLimit
				+ ", engineeringUnits=" + engineeringUnits + ", chartColour="
				+ chartColour + ", pointLocator=" + pointLocator
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		if (ver == 1) {
			...
			// The spinwave changes were not correctly implemented, so we need
			// to handle potential errors here.
			try {
				pointLocator = (PointLocatorVO) in.readObject();
			} catch (IOException e) {
				// Turn this guy off.
	...
	public void jsonSerialize(Map<String, Object> map) {
		...
		map.put("intervalLoggingPeriodType", Common.TIME_PERIOD_CODES
				.getCode(intervalLoggingPeriodType));
		map.put("intervalLoggingType", INTERVAL_LOGGING_TYPE_CODES
				.getCode(intervalLoggingType));
		...
		map.put("engineeringUnits", ENGINEERING_UNITS_CODES
				.getCode(engineeringUnits));
	}
	...
	public void jsonDeserialize(JsonReader reader, JsonObject json)
		...
		if (text != null) {
			...
			if (purgeType == -1)
				throw new LocalizableJsonException("emport.error.invalid",
						"purgeType", text, Common.TIME_PERIOD_CODES
								.getCodeList());
		}
