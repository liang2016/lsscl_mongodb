...
<script type="text/javascript">
  ...
  function TextRendererEditor() {
      ...
      this.change = function() {
    	  ...
    	  var arrs = new Array();
    	  arrs[0]="textRendererAnalog";
    	  arrs[1]="textRendererBinary";
    	  arrs[2]="textRendererMultistate";
    	  arrs[3]="textRendererNone";
    	  arrs[4]="textRendererPlain";
    	  arrs[5]="textRendererRange";
    	  arrs[6]="textRendererTime";
    	  for(var temp=0;temp<arrs.length;temp++){
    	  		if(arrs[temp]==currentTextRenderer){
    	  			dojo.html.show($(arrs[temp]));
    	  		}else{
    	  			dojo.html.hide($(arrs[temp]));
    	  		}
    	  }
      };
      ...
      this.save = function(callback) {
          ...
          if (typeName == "textRendererAnalog"){
              ACPTypeAttrDwr.setAnalogTextRenderer($get("textRendererAnalogFormat"),$get("textRendererAnalogSuffix"), callback);
          }else if (typeName == "textRendererBinary"){
              ACPTypeAttrDwr.setBinaryTextRenderer($get("textRendererBinaryZero"), dojo.widget.byId("textRendererBinaryZeroColour").selectedColour, $get("textRendererBinaryOne"),dojo.widget.byId("textRendererBinaryOneColour").selectedColour, callback);
          }else if (typeName == "textRendererMultistate"){
              ACPTypeAttrDwr.setMultistateRenderer(multistateValues, callback);
          }else if (typeName == "textRendererNone"){
              ACPTypeAttrDwr.setNoneRenderer(callback);
          }else if (typeName == "textRendererPlain"){
              ACPTypeAttrDwr.setPlainRenderer($get("textRendererPlainSuffix"), callback);
          }else if (typeName == "textRendererRange"){
              ACPTypeAttrDwr.setRangeRenderer($get("textRendererRangeFormat"), rangeValues, callback);
          }else if (typeName == "textRendererTime"){
              ACPTypeAttrDwr.setTimeTextRenderer($get("textRendererTimeFormat"),$get("textRendererTimeConversionExponent"), callback);
          }else{
              callback();
          }
      };
	 ...
	 this.removeAllMultistateValue = function() {
      	if(multistateValues.length>1)
		  multistateValues.splice(0,multistateValues.length);
        this.refreshMultistateList();
        return false;
      };
      ...
      this.removeRangeValue = function(theFrom, theTo) {
       ...
       this.removeAllRangeValue = function() {
