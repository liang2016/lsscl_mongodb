        ...
        if (reportConfig.isEmail()) {
          ...
          //  List<DataPointVO> pointNames=reportDao.getPointsByInstanceId(reportInstance.getId());
            ...
            if (creator.getImageData() != null){
            	if(isExchange){
            		ExchangeAttachment ea=new ExchangeAttachment(inlinePrefix + ReportChartCreator.IMAGE_CONTENT_ID,true,
                            creator.getImageData()); 
                        	attachments.add(ea);
            	}else{
                emailContent
            ...
            for (PointStatistics pointStatistics : creator.getPointStatistics()) {
                if (pointStatistics.getImageData() != null){
                	if(isExchange){
                		ExchangeAttachment image=new ExchangeAttachment(inlinePrefix
                                + pointStatistics.getChartName(),true,pointStatistics.getImageData()); 
            ...
            try {
                ...
                if(isExchange){
                    ...
                    Content exchangeContent = new Content(lm.getLocalizedMessage(bundle),creator.getHtml(),creator.getHtml(),null,null);
