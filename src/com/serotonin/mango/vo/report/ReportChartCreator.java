...
public class ReportChartCreator {
    ...
    public void createContent(ReportInstance reportInstance, ReportDao reportDao, String inlinePrefix,
        ...
        StreamHandler handler = new StreamHandler(reportInstance.getReportStartTime(),
                reportInstance.getReportEndTime(), IMAGE_WIDTH, createExportFile, bundle,pointNames);
        // Process the report content with the handler.
        if(createExportFile)
        	reportDao.reportInstanceData(reportInstance.getId(), handler);//做报表
        reportDao.reportInstanceData2(reportInstance.getId(), handler);//画统计图
    ...
    class StreamHandler implements ReportDataStreamHandler, DataQuantizerCallback {
        ...
        public StreamHandler(long start, long end, int imageWidth, boolean createExportFile, ResourceBundle bundle,List<DataPointVO> pointNames) {
            ...
            try {
                if (createExportFile) {
                    ...
                    reportCsvStreamer = new ReportCsvStreamer(new PrintWriter(new FileWriter(exportFile)), bundle,pointNames);
                }
            }
            catch (IOException e) {
...
//                reportCsvStreamer.pointData(rdv);
        ...
        public void pointData(ReportDataValue rdv,ReportDataValue oleRdv) {
//            if (quantizer != null)
//                quantizer.data(rdv.getValue(), rdv.getTime());
//            for(int i=0;i<this.pointStatistics.size();i++){
//             	if(this.pointStatistics.get(i).getReportPointId()==rdv.getReportPointId())
//             		this.pointStatistics.get(i).getStats().addValueTime(rdv);
//            }
            
            if (reportCsvStreamer != null)
                reportCsvStreamer.pointData(rdv,oleRdv);
        }
