   ...
   var dd = function(element, options)
   {
		...
		if(options.useSprite!=false) {
		 ...
		 if(options.jsonTitle==true && isJson==true) {			 
				 if(arrow.length!=0) {
					 ...
					 arrow = (img.length==0) ? "" : '<img src="'+img+'" align="absmiddle" onclick="gotoPage(\''+ elementid+'\',\''+$(currentOptOption).val()+'\')"/> ';
				 };
		 } else {
			 arrow = (arrow.length==0) ? "" : '<img src="'+arrow+'" align="absmiddle" onclick="gotoPage(\''+ elementid+'\',\''+$(currentOptOption).val()+'\')"/> ';
		 };
		};
	...
	var createTitleDiv = function () {
		...
		if(getElement(elementid).options.length>0) {
			...
			svalue = $("#"+elementid+" option:selected").val();
		};
		var img = "";
		var t = "";
		var reg = new RegExp(/^\{.*\}$/);
		...
		if(options.jsonTitle==true && isJson==true) {			 
			 if(arrow.length!=0) {
				 ...
				 img = (typeof obj[0].image=="undefined") ? "" : obj[0].image;
				 t = (typeof obj[0].title=="undefined") ? "" : obj[0].title;
				 arrow = (img.length==0 || options.showIcon==false || options.useSprite!=false) ? "" : '<img src="'+img+'" align="absmiddle" onclick="gotoPage(\''+ elementid+'\',\''+svalue+'\')"/> ';
			 }; 
		} else {
			arrow = (arrow.length==0 || arrow==undefined || options.showIcon==false || options.useSprite!=false) ? "" : '<img src="'+arrow+'" align="absmiddle" onclick="gotoPage(\''+ elementid+'\',\''+svalue+'\')"/> ';
		};
	...
	var createDropDown = function () {
		...
		var childid = getPostID("postChildID");
		var arrowid = getPostID("postArrowID");
		var iWidth = $("#"+elementid).outerWidth() + 20;
		var sStyle = options.style;
		if($("#"+id).length>0) {
			$("#"+id).remove();
