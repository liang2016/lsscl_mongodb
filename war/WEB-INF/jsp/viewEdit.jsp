...
<tag:page dwr="ViewDwr" onload="doOnload" js="view">
  ...
  <script type="text/javascript">
    ...
    function doOnload() {
        ...
        ViewDwr.editInit(function(result) {
           ...
           // mango.share.writeSharedUsers(result.viewUsers);
            ...
            linkEditor.addOptions(result.views);
    ...
    function createViewComponent(viewComponent, center) {
        ...
        else if(viewComponent.defName == 'link')
        	content = $("linkTemplate").cloneNode(true);
        else
        ...
        if (center) updateViewComponentLocation(content.id);
    }
    ...
    function configureComponentContent(content, viewComponent, parent, center) {
        ...
        if(viewComponent.defName == "link")
        	updateLinkComponentContent(content.id, viewComponent.content,viewComponent.viewId,viewComponent.factoryId);
        show(content);
    ...
    function updateLinkComponentContent(id, content,viewId,factoryId) {
        if (!content || content == "")
            $set(id +"Content", '<img src="images/html.png" alt=""/>');
        else
            $set(id +"Content",'<b><a  href="views.shtm?viewId='+viewId+'&factoryId='+factoryId+'">'+content+'</a></b>');
    }
    function openStaticEditor(viewComponentId) {
        closeEditors();
        staticEditor.open(viewComponentId);
    }
    function openLinkEditor(viewComponentId){
     	closeEditors();
        linkEditor.open(viewComponentId);
    }
    ...
    <table width="100%" cellspacing="0" cellpadding="0">
      <tr>
        <td>
            ...
            <tr>
              <td colspan="3">
                <div id="viewContent" class="borderDiv" style="left:0px;top:0px;float:left;
                  ...
                  <%@ include file="/WEB-INF/jsp/include/linkEditor.jsp" %>
          ...
          <div id="linkTemplate" onmouseover="showLayer('c'+ getViewComponentId(this) +'Controls');"
                  onmouseout="hideLayer('c'+ getViewComponentId(this) +'Controls');"
                  style="position:absolute;left:0px;top:0px;display:none;">
            <div id="c_TEMPLATE_Content"></div>
            <div id="c_TEMPLATE_Controls" class="controlsDiv">
              <table cellpadding="0" cellspacing="1">
                <tr><td><tag:img png="pencil" onclick="openLinkEditor(getViewComponentId(this))"
                        title="viewEdit.editStaticView"/></td></tr>
                <tr><td><tag:img png="html_delete" onclick="deleteViewComponent(getViewComponentId(this))"
                        title="viewEdit.deleteStaticView"/></td></tr>
              </table>
