...
public class ReportsDwr extends BaseDwr {
    public DwrResponseI18n init() {
        ...
        response.addData("points", getReadablePointsByScope());
        response.addData("mailingLists", new MailingListDao().getMailingLists(user.getCurrentScope().getId()));
        response.addData("users", new UserDao().getUsers(user.getCurrentScope().getId()));
        response.addData("reports", reportDao.getReports(user.getId(),user.getCurrentScope().getId()));
        response.addData("instances", getReportInstances(user,user.getCurrentScope().getId()));
    ...
    public DwrResponseI18n saveReport(int id,int factoryId, String name, List<ReportPointVO> points, int includeEvents,
            ...
            String scheduleCron, boolean email, boolean includeData, boolean zipData,boolean intervalTime,int timeType,int timeNum,
    ...
    public DwrResponseI18n runReport(String name,int factoryId, List<ReportPointVO> points, int includeEvents,
            boolean includeUserComments, int dateRangeType, int relativeDateType, int previousPeriodCount,
            int previousPeriodType, int pastPeriodCount, int pastPeriodType, boolean fromNone, int fromYear,
            int fromMonth, int fromDay, int fromHour, int fromMinute, boolean toNone, int toYear, int toMonth,
            int toDay, int toHour, int toMinute, boolean email, boolean includeData, boolean zipData,boolean intervalTime,int timeType,int timeNum,
            List<RecipientListEntryBean> recipients) {
        DwrResponseI18n response = new DwrResponseI18n();
    ...
    public List<ReportInstance> deleteReportInstance(int instanceId) {
        ...
        return getReportInstances();
    }

    public List<ReportInstance> getReportInstances() {
    	User user = Common.getUser();
    	int factoryId=user.getCurrentScope().getId();
        return getReportInstances(user,factoryId);
    }

    private List<ReportInstance> getReportInstances(User user,int factoryId) {
        List<ReportInstance> result = new ReportDao().getReportInstances(user.getId(),factoryId);
